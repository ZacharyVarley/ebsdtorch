...
from ebsdtorch.utils.pca_covmat import OnlineCovMatrix
...
def pca_dictionary_index_orientations(
    ...
    for i, mp in enumerate(master_patterns):
        ...
        pca_matrix = eigenvectors[:, -n_pca_components : (-n_pca_components + 300)]

        # calc dictionary (if the learning and indexing resolutions are different)
        if dictionary_resolution_learn_deg != dictionary_resolution_index_deg:
            # get an orientation dictionary
            ori_tensor = sample_ori_fz_laue_angle(
                laue_id=mp.laue_group,
...
exp_pats = (
    torch.tensor(kp.data.ni_gain(number=5, allow_download=True).data)
    .to(device)
    .to(torch.float32)
)
...
pca_dictionary_index_orientations(
    ...
    geom,
    exp_pats,
    dictionary_resolution_learn_deg=0.5,
    dictionary_resolution_index_deg=0.2,
    dictionary_chunk_size=4096,
    signal_mask=mask,
    virtual_binning=1,
    experiment_chunk_size=4096,
