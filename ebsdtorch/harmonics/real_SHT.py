...
print(f"Generating random complex signal on the DH grid with bandlimit {B}")
Psi = torch.view_as_complex(
    2 * (torch.rand(1024, 2 * B - 1, B, 2, device=device).double() - 0.5)
).to(device)

print(f"Setting SH coefficients to zero for l < |m|")
...
print(f"Instantiating FTSHT and ITSHT")

F = FTSHT(B).to(device)
I = ITSHT(B).to(device)

print(f"Running forward and inverse transform")

Psi2 = F(I(Psi))

print(f"Comparing forward and inverse transform")

## This error should be very, very  small
error = torch.mean(torch.abs(Psi - Psi2))
...
error = torch.mean(torch.abs(Psi_f - Psi2_f))
...
# Psi_f = torch.view_as_complex(2 * (torch.rand(4096, B, B + 1, 2).double() - 0.5)).to(
#     device
# )
...
# sht = RealSHT(128, 256, grid="equiangular").to(device)
# isht = InverseRealSHT(128, 256, grid="equiangular").to(device)

# Psi3 = isht(Psi_f)
# print(Psi3.shape, Psi3.dtype)
# Psi4 = sht(Psi3)

# ## This error should be very, very  small
# error = torch.mean(torch.abs(Psi_f - Psi4))
# print(f"FP32 Error: {error}")


# # start with real signal and to forward and inverse transform
# Psi_f = 1 + (torch.rand(4096, B, 2 * B).double() - 0.5).to(device)
